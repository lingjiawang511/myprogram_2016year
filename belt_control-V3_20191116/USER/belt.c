...
static u8 Start_Belt11(u8 step)
{
    ...
    switch (step) {
        case 0x00:
            Belt11_Speed_Control(step);
            step++;
            belt11.belt_actual_state = 1;
            break;
        case 0x01:
            Belt11_Speed_Control(step);
            start_time++;
...
static u8 Start_Belt12(u8 step)
{
    ...
    switch (step) {
        case 0x00:
            Belt12_Speed_Control(step);
            step++;
            belt12.belt_actual_state = 1;
            break;
        case 0x01:
            Belt12_Speed_Control(step);
            start_time++;
...
{
    if ((READ_BELT11_SENSOR1 == 1) || (READ_BELT11_SENSOR2 == 1) || (belt11.comm_ctr_start == 1)) {
        delay_ms(10);
        if ((READ_BELT11_SENSOR1 == 1) || (READ_BELT11_SENSOR2 == 1) || (belt11.comm_ctr_start == 1)) {
...
void belt11_time_control(void)
{
    ...
    if ((belt11.stop_delay_state == 1)&&(belt11.comm_run_always_flag == 0)) {
        ...
        if (stop_delay_time >= belt11.comm_ctr_run_time) {
            belt11.stop_ok = 1;
            belt11.stop_delay_state = 0;
            stop_delay_time = 0;
        }
...
void belt12_time_control(void)
{
    ...
    if ((belt12.stop_delay_state == 1)&&(belt12.comm_run_always_flag == 0)) {
        ...
        if (stop_delay_time >= belt12.comm_ctr_run_time) {
            belt12.stop_ok = 1;
            belt12.stop_delay_state = 0;
            stop_delay_time = 0;
        }
