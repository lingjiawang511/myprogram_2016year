...
//-----------------------------------------------------------------------------
#define SHT3X_RESET_IO     GPIO_Pin_8	  
#define SHT3X_ALERT_IO     GPIO_Pin_9	  
#define SHT3X_RESET_SOURE  GPIOB
#define SHT3X_ALERT_SOURE  GPIOB
// Reset on port B, bit 12
#define RESET_LOW()  GPIO_ResetBits(SHT3X_RESET_SOURE,SHT3X_RESET_IO) // set Reset to low
#define RESET_HIGH() GPIO_SetBits(SHT3X_RESET_SOURE,SHT3X_RESET_IO)  // set Reset to high

// Alert on port B, bit 10
#define ALERT_READ   GPIO_ReadInputDataBit(SHT3X_ALERT_SOURE,SHT3X_ALERT_IO)    // read Alert
//=============================================================================
...
void SHT3X_Init(u8t i2cAddress)          /* -- adapt the init for your uC -- */
{
    ...
    GPIO_InitStructure.GPIO_Pin=SHT3X_RESET_IO;
    GPIO_InitStructure.GPIO_Speed=GPIO_Speed_2MHz;
    GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_OD;
    GPIO_Init(SHT3X_RESET_SOURE,&GPIO_InitStructure);
    
    GPIO_InitStructure.GPIO_Pin=SHT3X_ALERT_IO;
    GPIO_InitStructure.GPIO_Mode=GPIO_Mode_IN_FLOATING;
    GPIO_Init(SHT3X_ALERT_SOURE,&GPIO_InitStructure);  
...
static etError I2c_WaitWhileClockStreching(u8t timeout)
{
  ...
  I2C_SCL_IN();
  while(GPIO_ReadInputDataBit(GPIO_I2C,I2C_SDA) == 0)
  {
    if(timeout-- == 0) {
        I2C_SCL_OUT();
...
etError I2c_WriteByte(u8t txByte)
{
  ...
  {
    if((mask & txByte) == 0) I2C_SDA_L; // masking txByte, write bit to SDA-Line
    else                     I2C_SDA_H;
    delay_nus(1);               // data set-up time (t_SU;DAT)
    I2C_SCL_H;                         // generate clock pulse on SCL
    delay_nus(5);               // SCL high time (t_HIGH)
    I2C_SCL_L;
    delay_nus(1);               // data hold time(t_HD;DAT)
  }
  I2C_SDA_IN();                           // release SDA-line
  I2C_SCL_H;                           // clk #9 for ack
  delay_nus(5);                 // data set-up time (t_SU;DAT)
  if(GPIO_ReadInputDataBit(GPIO_I2C,I2C_SDA)){
      error = ACK_ERROR;       // check ack from i2c slave
  }
  I2C_SCL_L;
  SDA_OPEN();
  delay_nus(20);                // wait to see byte package on scope
...
etError I2c_ReadByte(u8t *rxByte, etI2cAck ack, u8t timeout)
{
  ...
  *rxByte = 0x00;
  ...
  { 
    ...
    delay_nus(10);                // clock set-up time (t_SU;CLK)
//     error = I2c_WaitWhileClockStreching(timeout);// wait while clock streching
//     delay_nus(3);                // SCL high time (t_HIGH)
    if(GPIO_ReadInputDataBit(GPIO_I2C,I2C_SDA)){
        *rxByte |= mask;        // read bit
    }
    I2C_SCL_L;
    delay_nus(2);                // data hold time(t_HD;DAT)
  }
  I2C_SDA_OUT();
  if(ack == ACK) I2C_SDA_L;              // send acknowledge if necessary
  else           I2C_SDA_H;
  delay_nus(10);                  // data set-up time (t_SU;DAT)
  I2C_SCL_H;                            // clk #9 for ack
  delay_nus(10);                  // SCL high time (t_HIGH)
  I2C_SCL_L;
  I2C_SDA_H;                            // release SDA-line
  delay_nus(20);                 // wait to see byte package on scope
...
etError SHT3x_ReadSerialNumber(u32t* serialNumber)
{
  ...
  delay_nus(500);//写完命令后必须要500US等待才可以读数据
...
etError SHT3X_SoftReset(void)
{
  ...
  if(error == NO_ERROR) delay_nus(50000); 
